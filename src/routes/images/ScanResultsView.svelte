...
{#if results.length === 0}
	<div class="text-sm text-muted-foreground">{m.images_scan_no_results()}</div>
{:else}
			...
			<div class="flex flex-wrap items-center gap-1.5 shrink-0">
				{#if activeResult.summary.critical > 0}
					<Badge variant="outline" class="bg-red-500/10 text-red-500 border-red-500/30 text-xs py-0">
						{activeResult.summary.critical} {m.images_scan_critical()}
					</Badge>
				{/if}
				{#if activeResult.summary.high > 0}
					<Badge variant="outline" class="bg-orange-500/10 text-orange-500 border-orange-500/30 text-xs py-0">
						{activeResult.summary.high} {m.images_scan_high()}
					</Badge>
				{/if}
				...
				{#if activeResult.summary.negligible > 0}
					<Badge variant="outline" class="bg-gray-500/10 text-gray-500 border-gray-500/30 text-xs py-0">
						{activeResult.summary.negligible} {m.images_scan_negligible()}
					</Badge>
				{/if}
				{#if activeResult.summary.unknown > 0}
					<Badge variant="outline" class="bg-gray-500/10 text-gray-500 border-gray-500/30 text-xs py-0">
						{activeResult.summary.unknown} {m.container_inspect_unknown()}
					</Badge>
				{/if}
					...
					<table class="w-full text-xs">
						<thead class="bg-muted sticky top-0">
							<tr>
								...
								<th class="text-left py-1.5 px-2 font-medium w-[18%]">{m.images_scan_installed()}</th>
								<th class="text-left py-1.5 px-2 font-medium w-[20%]">{m.container_batch_fixed()}</th>
							</tr>
						</thead>
						<tbody>

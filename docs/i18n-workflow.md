...
- **Docker 网络驱动/模式名保持英文**：`Bridge` `Host` `Overlay` `Macvlan` `IPvlan` `None` `Private` `VEPA` `Passthru` `L2` `L3` `L3S`。
...
- **预去重（加 key 前就做）**：把要新增的英文 value 先扫一遍 `en.json`，若已有同 value 同中文的 key，直接用已有 key，不要新增。常见容易重复的通用词：`Prune`、`Refresh`、`Cancel`、`Clear`、`Delete`、`Remove`、`View details`、`Driver`、`Scope`、`Subnet`、`Gateway`、`Labels`、`Options`、`IPAM configuration`、`Internal network`、`External network`、`Attachable`、`Service`、`Tree/Grid/Circle/Radial/Force`、`N/A`。
...
- 新 key 按区域加前缀：`settings_env_*`、`settings_env_modal_*`、`settings_env_updates_*`、`settings_env_activity_*`、`settings_env_event_*`、`networks_*`（对齐 general tab 的 `settings_general_*`）。
- **加 key 方式**：用脚本以 `json.dumps(key)` + `json.dumps(value)` 拼接向两个 JSON **追加**（保 TAB 格式），**注意值里的双引号会被自动转义**。**不要** `JSON.stringify` 整文件（会打乱缩进/顺序、毁掉 diff）。en 填英文原文，zh 填中文。一个文件的 key **一次性批量追加**，别分多轮反复读写、反复对齐。
...
2. **首选 `perl -pi -e` 配 `#` 分隔符**（`{}` 分隔符会和替换串里的 `{m...}` 冲突报 "pattern not terminated"）：
   ...
   - 正则里转义 `(` `)` `.` `?`；`'` 用 `.` 匹配（如 `won.t`）。
3. **复杂多分支块**（带复数 `{#if}` 的对话框、graph details panel 等）：用 Python 按**行号区间**或**完整大段块**整体替换，比逐行手改稳。替换前用 `cat -A` 确认块内每行 tab 层级，避免破坏 `{#if}`/`{:else}` 结构。
4. 同一英文多处映射同 key → 用 `g` 标志一次替换。
5. **模块级常量无法访问 `m`**：`<script lang="ts" module>` 里的静态数组若包含需要翻译的文本，必须移到组件级 `<script>` 中，并把字段改成 getter，如 `description: () => m.xxx()`。`Select.Item` 的 `label` 可以接受函数调用结果。
6. **专有名词选项值保持原样但可译附加说明**：如 `Bridge (default)` 可译为 `Bridge（默认）`，但核心术语 `Bridge` 保留英文。
...
# 2) 组件里所有 m.xxx() 的 key 是否都存在（防拼写错）
python3 -c "
import json,re
from pathlib import Path
e=json.load(open('src/lib/i18n/messages/en.json'))
for p in Path('src').rglob('*.svelte'):
    keys=set(re.findall(r'\bm\.(\w+)\(', p.read_text()))
    missing=keys-set(e)
    if missing: print(p, sorted(missing))
"

# 3) 残留英文
for f in src/routes/<目标目录>/*.svelte; do
  grep -nE '>[A-Za-z][A-Za-z ]{2,}<|placeholder="[A-Z]|title="[A-Z]' "$f" | grep -vE 'm\.|Grype|Trivy|Hawser|HTTP|class=|</|Docker|Bridge|Host|Overlay|Macvlan|IPvlan|None|Private|VEPA|Passthru|L2|L3|L3S|default|true|false'
done

# 4) 类型检查（只看自己改的文件，server/api 既有报错忽略）
# npm run check 全项目较慢且可能超时，可直接跑 svelte-check 再 grep
timeout 120 npx svelte-check --tsconfig ./tsconfig.json > /tmp/check.log 2>&1
grep -A2 -B1 "src/routes/<目标目录>/" /tmp/check.log
```
...
3. **验证 1-3 拼成一条命令**（对齐 / key 存在 / 残留英文，全是毫秒级），最后**整文件收尾才跑一次** `npx svelte-check`（全项目扫十几秒，别中途试探性地跑）。

**翻译期停掉 `npm run dev`**：它的 watcher + 同 pts 输出是之前终端串字符、Edit 失败的元凶。汉化不需要热更新，`npx kill-port 5173 5174` 停掉，要预览再起。

## 进度

`src/routes` 下约 118 个 `.svelte`，目前仅约 11 个已接入翻译。已译页面可作样板对照：`login`、`profile`、`settings/general`、`settings/environments` 全套、`containers`、`networks` 全套、`app-sidebar`、`CommandPalette`。其余（containers 子页、images、volumes、stacks 等）尚待汉化。
